...
## 2. Problem Statement
Farmers in Pakistan often treat symptoms (diseases) without addressing the cause (pests), leading to recurring crop failure and wasted money on incorrect pesticides. Existing apps (like PlantVillage) use US-centric data that fails in local Pakistani field conditions.
...
- **Model:** EfficientNetB3 trained on combined PlantVillage/PlantCity datasets.
...
## 7. Success Metrics (FYP Defense)
- **Model Accuracy:** >85% on validation set.
- **Latency:** <3 seconds for inference.
